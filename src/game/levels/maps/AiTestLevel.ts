 ...
 *
 * Layout: cuadrado plano de 64×64m centrado en (0,0), dividido en 4 zonas
 * que comparten el spawn del player. Cada zona prueba un aspecto distinto:
 *
 *  NW (combine zone) ──── NE (casa 2 pisos)
 *         │                       │
 *  SW (zombie corridors) ── SE (obstacle gym)
 *
 *  - Combine zone: arena abierta con cover medio + plataforma elevada con rampa.
 *  - Casa 2 pisos: interior + balcón exterior + rampa al techo, para verificar
 ...
 *    pilares y crates para que el cover system encuentre opciones heterogéneas.
 *
 ...
 * `stepOffset: 0.4` y el NavGraphBuilder `maxStepHeight: 1.4`, así que
 * 0.3m queda holgado para ambos.
 */

/**
 * Importante: `NavGraphBuilder.isWalkableBox` requiere `size.y <= 0.75`. Si
 * el slab del piso fuera más grueso, no se generarían nodos arriba — los
 * NPCs no podrían pathear. Mantener fino.
 */
const SLAB_THICKNESS = 0.4;
// Y de la superficie superior del piso principal. El slab `aitest-floor` está
// centrado en Y=-SLAB_THICKNESS/2, así que su cara superior cae en Y=0. Las
// rampas que arrancan desde el suelo deben usar este valor — si arrancan más
...
const FLOOR: StaticBoxDefinition[] = [
  ...
  // Perímetro: paredes altas que delimitan el mapa.
  { id: 'aitest-boundary-n', position: [0, 1.5, -32], size: [64, 3, 0.4], material: 'wall' },
  { id: 'aitest-boundary-s', position: [0, 1.5, 32], size: [64, 3, 0.4], material: 'wall' },
  { id: 'aitest-boundary-e', position: [32, 1.5, 0], size: [0.4, 3, 64], material: 'wall' },
  { id: 'aitest-boundary-w', position: [-32, 1.5, 0], size: [0.4, 3, 64], material: 'wall' },
];

// ── NW: Combine zone (-X, -Z) ──────────────────────────────────────────────
const COMBINE_ZONE: StaticBoxDefinition[] = [
  // Cuatro coberturas medias (chest-high) dispersas.
  { id: 'cz-cover-1', position: [-22, 0.7, -10], size: [3, 1.4, 0.6], material: 'brick' },
  { id: 'cz-cover-2', position: [-12, 0.7, -18], size: [0.6, 1.4, 3], material: 'brick' },
  { id: 'cz-cover-3', position: [-18, 0.7, -25], size: [3, 1.4, 0.6], material: 'brick' },
...
// ── NE: Casa de 2 pisos (+X, -Z) ───────────────────────────────────────────
// Centro de la casa en (18, ?, -18). Footprint 14×14. Piso 1 en y=0.5, piso 2 en y=3.5.
...
const HOUSE: StaticBoxDefinition[] = [
  ...buildHouse(HOUSE_SHELL_1),
  ...buildHouse(HOUSE_SHELL_2),
  // Losa del piso 2 compuesta por 2 sub-slabs que dejan un hueco de ~13×3.5m
...
const ZOMBIE_ZONE: StaticBoxDefinition[] = [
  // Pared larga horizontal (corta el área en 2 corridors).
  ...
  { id: 'zz-pillar', position: [-18, 1.25, 24], size: [1, 2.5, 1], material: 'brick' },
];
...
const OBSTACLE_ZONE: StaticBoxDefinition[] = [
  ...
  { id: 'oz-half-1', position: [10, 0.3, 8], size: [3, 0.6, 0.4], material: 'brick' },
  { id: 'oz-half-2', position: [20, 0.3, 12], size: [0.4, 0.6, 3], material: 'brick' },
  { id: 'oz-half-3', position: [14, 0.3, 24], size: [4, 0.6, 0.4], material: 'brick' },
  // Mid-walls (chest-high cover): 1.2m alto.
  { id: 'oz-mid-1', position: [18, 0.6, 8], size: [3, 1.2, 0.4], material: 'brick' },
  { id: 'oz-mid-2', position: [24, 0.6, 18], size: [0.4, 1.2, 4], material: 'brick' },
  { id: 'oz-mid-3', position: [12, 0.6, 18], size: [3, 1.2, 0.4], material: 'brick' },
  // Full walls (bloquean LOS completo): 2.4m alto, segmentos cortos.
  { id: 'oz-full-1', position: [25, 1.2, 25], size: [4, 2.4, 0.4], material: 'brick' },
  { id: 'oz-full-2', position: [10, 1.2, 14], size: [0.4, 2.4, 4], material: 'brick' },
  // Pilares (1m square).
  { id: 'oz-pillar-1', position: [16, 1.25, 14], size: [1, 2.5, 1], material: 'brick' },
  { id: 'oz-pillar-2', position: [22, 1.25, 22], size: [1, 2.5, 1], material: 'brick' },
];
...
export const AiTestLevel: LevelDefinition = {
  ...
  title: 'Sandbox IA',
  description:
    'Mapa de debug. 4 zonas: combines con cover, casa de 2 pisos, corredores para zombies y gimnasio de obstáculos. Pocos NPCs.',
  background: 0x1a1f24,
  sun: {
  ...
  dynamicBoxes: [
    ...
    { id: 'aitest-crate-1', position: [-8, 1, -8], size: [0.9, 0.9, 0.9], mass: 1.4, material: 'crate' },
    { id: 'aitest-crate-2', position: [-9, 1, -9], size: [0.9, 0.9, 0.9], mass: 1.4, material: 'crate' },
    { id: 'aitest-crate-3', position: [8, 1, 8], size: [1.1, 1.1, 1.1], mass: 1.8, material: 'crate' },
    { id: 'aitest-barrel-1', position: [15, 1, 22], size: [0.7, 1.2, 0.7], mass: 1.0, material: 'hazard' },
  ],
  doors: [],
  npcs: [
    // 2 combines en la zona combine — uno cerca, otro en plataforma alta.
    { id: 'ai-combine-ground', position: [-20, 2, -12], characterId: 'combine' },
    { id: 'ai-combine-platform', position: [-20, 3.5, -28], characterId: 'combine' },
    // 1 combine en el balcón de la casa (test cover elevado + escalera).
    { id: 'ai-combine-balcony', position: [18, 4.5, -11], characterId: 'combine' },
  ...
  weaponPickups: [
    ...
    { id: 'ai-pickup-gravity-gun', weaponId: 'gravityGun', position: [3, 0.7, 2] },
    { id: 'ai-pickup-grenade-1', weaponId: 'grenade', position: [-2, 0.7, 3] },
    { id: 'ai-pickup-grenade-2', weaponId: 'grenade', position: [-1.5, 0.7, 3] },
