...
export interface RangedFireContext {
  origin: Vector3;
  /** Posición del target. El combat aplica spread y aim error. */
  targetPosition: Vector3;
  /** Body del NPC dueño, para excluirlo del raycast. */
  ownerBody: RAPIER.RigidBody;
  /** Tiempo elapsed del juego. */
  now: number;
  /**
   * 0..1: progreso de asentamiento de la mira. 0 = recién detectó / disparo
   * desde el frío, max aim error. 1 = mira asentada, min aim error.
...
/**
 * Combate ranged genérico para NPCs.
 *
 * Lee stats de daño/spread/range/fireRate del `WeaponDefinition` del juego
 * y comportamiento táctico (burst size, pausa entre ráfagas, aim error,
 * reaction time) del `CharacterRangedAttackConfig`. No es un `Weapon` —
 * los NPCs no comparten cooldowns ni view-model con el player.
 *
 * Patrón de disparo: cuando se le ordena fire, dispara una ráfaga de
 * `burstSize` shots espaciados por `1/weapon.fireRate`. Entre ráfagas
 * espera `pauseBetweenBursts`. Cuando el magazine se agota, debe
 * llamarse `reload()` desde fuera (la FSM decide cuándo).
 */
export class NpcRangedCombat {
  ...
  /** True si el NPC puede iniciar una nueva ráfaga ahora (no en reload, cooldown ok, ammo). */
  ...
  /** Inicia una ráfaga. Devuelve false si no se puede. */
  ...
  /** Detiene la ráfaga actual (e.g. perdió LOS, target murió). */
  abortBurst(): void {
    this.burstShotsLeft = 0;
  }
