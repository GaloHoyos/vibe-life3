import { BoxGeometry, Group, Mesh, MeshStandardMaterial, Quaternion, Scene, Vector3, type Object3D } from 'three';
...
export class WeaponViewModel {
  ...
  update(delta: number, cameraSystem: CameraSystem, recoil: Recoil, speed: number): void {
    if (!this.equipped) {
      return;
    }
    ...
    offset.y += Math.sin(this.bobTime * 7) * 0.008 * Math.min(speed, 1) - recoil.offset.y;
    offset.z += Math.cos(this.bobTime * 5) * 0.006 * Math.min(speed, 1);

    this.root.position.copy(cameraSystem.camera.localToWorld(offset));
    const rotation = new Quaternion().setFromEuler(this.equipped.viewModelRotation);
    this.root.quaternion.copy(cameraSystem.camera.quaternion).multiply(rotation);
...
function createFallbackModel(definition: WeaponDefinition): Object3D {
  const mesh = new Mesh(
    ...
    new MeshStandardMaterial({ color: definition.type === 'special' ? 0x3aa7d8 : 0x24282c, roughness: 0.6 }),
  );
