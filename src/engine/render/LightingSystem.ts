...
export interface SunOptions {
  /**
   * Dirección desde la escena hacia el sol (cualquier magnitud, se normaliza).
   * `[0, 1, 0]` = mediodía, `[1, 0.3, 0]` = atardecer bajo al este.
   */
  direction?: VectorTuple;
  /** Color del sol. Default cálido neutro `0xfff2d6`. */
  color?: number;
...
/**
 * Iluminación canónica del juego. Pensado para correr con IBL (HDRI vía
 * `EnvironmentSystem`) — por eso ambient + hemisphere quedan en fill bajo
