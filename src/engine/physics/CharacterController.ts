 ...
 * (en `game/`) y se pasa este struct por frame — así `engine/` no conoce
 * acciones específicas del juego ni sus bindings.
 */
 ...
 * NO se reemplaza por frame, así que en el aire se preserva el momentum y el
 * input solo ajusta la dirección dentro de un `maxAirWishSpeed` cap.
 */
export class CharacterController extends KinematicCharacterBase {
  ...
  private updateCrouch(delta: number): void {
    ...
    // Crouch jump (HL1/HL2): en el aire NO trasladamos el cuerpo, así los pies
    // suben por la cápsula achicada y se gana clearance para subir bordes que
    // un salto normal no alcanza. La cámara baja vía `eyeOffset` interpolado.
