...
export interface KinematicCharacterBaseOptions {
  ...
  position: Vector3;
  radius: number;
  /** Mitad de la sección cilíndrica de la cápsula (sin contar los hemisferios). */
  halfHeight: number;
  metadata: PhysicsMetadata;
  /** Offset interno del Rapier `KinematicCharacterController`. Default 0.03. */
  offset?: number;
  /** Auto-step máximo en altura. */
  stepOffset?: number;
  /** Auto-step distancia horizontal mínima. */
  stepMaxClimb?: number;
 ...
 *
 * Encapsula:
 *  - cuerpo `kinematic-positionBased` + cápsula
 ...
 *  - bucle estándar `computeColliderMovement → setNextKinematicTranslation`
 *
 * Las subclases definen *qué* velocidad asignan (input vs IA) y, opcionalmente,
 * la rotación del body (`setNextKinematicRotation`).
 */
