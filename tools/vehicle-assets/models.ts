...
import {
  ...
  roundedBox,
  wheel as buildWheel,
} from "../shared/gltf/geometry.js";
import type { LoftSection } from "../shared/gltf/geometry.js";
import {
  countNode,
  createAnchor,
  createGlassMaterial,
...
import type {
  AtlasTile,
  BuildContext as GltfBuildContext,
  Euler,
  GeneratedTextureSet,
  GeometryPart,
  LodStats,
...
import type { GeneratedVehicleStats, VehicleAssetSpec } from "./types.js";

/** El contexto genérico más el spec, que sólo consultan los builders de acá. */
interface BuildContext extends GltfBuildContext {
...
function buildRebelCrawler(context: BuildContext): void {
  const glassMaterial = createGlassMaterial(context.document, context.spec.id);
  for (const lod of [0, 1, 2] as const) {
    const root = createNode(context, context.sceneRoot, `visual_lod${lod}`, {
      extras: {
        kind: "vehicle-lod",
